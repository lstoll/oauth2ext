...
// Verifier returns an immutable verifier bound to the provider's stable key
// set and the supplied policy. Refreshes atomically update that key set.
func (p *Provider) Verifier(ctx context.Context, policy jwt.ValidationPolicy) (*jwt.Verifier, error) {
