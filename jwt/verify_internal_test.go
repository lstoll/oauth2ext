...
func newTestSignerWithType(t *testing.T, typ string) *testSigner {
	...
	local, err := NewSigningIdentity(key, ES256, "")
	if err != nil {
		t.Fatal(err)
	}
	...
	ks, err := ParseVerificationJWKS(jwks)
	if err != nil {
		t.Fatal(err)
	}
...
func (s *testSigner) signRaw(t *testing.T, payload []byte) string {
	...
	joseOpts.WithHeader("kid", s.local.KeyID())
...
func verifyJWT(t testing.TB, keys *VerificationKeySet, compact string, policy ValidationPolicy) (*VerifiedJWT, error) {
	t.Helper()
	verifier, err := NewVerifier(keys, policy)
	if err != nil {
		return nil, err
	}
	return verifier.Verify(compact)
}
