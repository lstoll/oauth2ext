...
func TestDPoPVerifier_ExampleToken(t *testing.T) {
	...
	result, err := dv.VerifyAndDecode(rfc9449ExampleToken)
	if err != nil {
...
func TestDPoPVerifier_RoundTrip(t *testing.T) {
	...
	// Create encoder using the constructor - it automatically creates the JWK
	...
	token, err := encoder.SignAndEncode(opts)
	if err != nil {
	...
	result, err := verifier.VerifyAndDecode(token)
	if err != nil {
		t.Fatalf("failed to verify and decode DPoP token: %v", err)
	}
	...
	// Verify thumbprint matches what we'd calculate from the encoder's JWK
...
func TestDPoPVerifier_RejectsMissingJWK(t *testing.T) {
	...
	token, err := encoder.encodeWithHeaders(rawJWT, nil)
	if err != nil {
	...
	verifier := &DPoPVerifier{}
	_, err = verifier.VerifyAndDecode(token)
	if err == nil {
...
func TestDPoPVerifier_HTM_HTU_Validation(t *testing.T) {
	...
	t.Run("Valid HTM and HTU", func(t *testing.T) {
		verifier := &DPoPVerifier{}
		_, err := verifier.VerifyAndDecodeWithOptions(token, &VerifyOptions{
			ExpectedHTM: "POST",
			ExpectedHTU: "https://server.example.com/token",
		})
		if err != nil {
	...
	t.Run("HTU mismatch", func(t *testing.T) {
		...
		_, err := verifier.VerifyAndDecodeWithOptions(token, &VerifyOptions{
			ExpectedHTM: "POST",
			ExpectedHTU: "https://other.example.com/token",
		})
		if err == nil {
