...
// DPoPProof is the result of verifying a DPoP token.
...
func (d *DPoPVerifier) VerifyAndDecode(compact string, validator *Validator) (*DPoPProof, error) {
	...
	// Firstly, make sure the thumbprint matches the expected one.
	...
	handle, err := jwt.JWKSetToPublicKeysetHandle([]byte(fmt.Sprintf(`{"keys":[%s]}`, string(jwkWithAlgJSON))))
	if err != nil {
